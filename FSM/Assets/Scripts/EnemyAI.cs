...
public class EnemyAI : MonoBehaviour
{
    ...
    private void Awake()
    {
        DataMgr.Instance.GetMonsterDataFromCSV(1);
        DataMgr.Instance.SaveGameData();
        ...
        Ctrl.PlayerAttack += AttackTogether;
    }
    ...
    void UpdateKnockback()
    {
        if (IsFindEnemy())
        {
            Ctrl.asd(transform.position, _targetPosition);
        }
    ...
    {
        _animator.SetTrigger(AnimID.ISKNOCKBACK);
    ...
    void AttackTogether(Vector3 friendlyPos, Vector3 targetPos)
    {
        if(Vector3.Distance(transform.position,friendlyPos)<1000f)
        {
